...
use interceptor::PacketInterceptor;
...
async fn main() -> anyhow::Result<()> {
    ...
    if config.experimental_hyper_http { info!("Experimental hyper HTTP pipeline: ENABLED"); } else { info!("Experimental hyper HTTP pipeline: disabled"); }
    
    // Create packet interceptor based on mode
    let interceptor = PacketInterceptor::new(config.interception_mode, config.listen_port)?;
    info!("Packet interceptor initialized in {:?} mode", config.interception_mode);
...
fn setup_logging(config: &TotanConfig) -> anyhow::Result<()> {
    ...
    if config.logging.format.eq_ignore_ascii_case("json") {
        base.with(fmt::layer().event_format(fmt::format().json())).init();
    } else {
...
fn load_config(args: &CliArgs) -> anyhow::Result<TotanConfig> {
    ...
    // Logging overrides
    if let Some(lvl) = &args.log_level { config.logging.level = lvl.clone(); }
    if let Some(fmt) = &args.log_format { config.logging.format = fmt.clone(); }
