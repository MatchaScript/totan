...
use crate::http_proxy::{HttpProxyContext, serve_http_connection};
...
impl ConnectionManager {
    ...
    pub async fn handle_connection(
        ...
        debug!(
            ...
            client_addr, stream.local_addr()?, original_dest
        );
        ...
        let scheme = if intercepted_conn.original_dest.port() == 443 { "https" } else { "http" };
        let default_port = if scheme == "https" { 443 } else { 80 };
        let authority = if intercepted_conn.original_dest.port() == default_port {
            hostname_for_url.clone()
        } else {
            format!("{}:{}", hostname_for_url, intercepted_conn.original_dest.port())
        };
        let target_url = format!("{}://{}/", scheme, authority);

        debug!("Target URL candidate: {}{}", target_url, sni_hostname.as_ref().map(|s| format!(" (SNI: {})", s)).unwrap_or_default());
        ...
        // Experimental hyper-based HTTP pipeline branch (plain HTTP only)
        if self.config.experimental_hyper_http
            && intercepted_conn.original_dest.port() == 80
            && upstream_proxy.is_some()
        {
            if let Some(proxy_url) = &upstream_proxy {
                match HttpProxyContext::new(intercepted_conn.clone(), proxy_url) {
                    ...
                    Err(e) => {
                        tracing::warn!("Failed to init hyper HTTP context: {} (falling back to legacy path)", e);
                    }
                }
            }
        }
...
fn get_original_destination(stream: &TcpStream) -> Result<SocketAddr> {
    ...
    #[cfg(not(target_os = "linux"))]
    {
        ...
        stream.peer_addr().map_err(|e| anyhow::anyhow!("Cannot get original destination: {}", e))
    }
}
