...
use url::Url;
use totan_common::{InterceptedConnection, TotanError, config::ErrorMitigationConfig};
...
impl UpstreamHandler {
    pub fn new(default_proxy: Option<String>, connect_timeout_ms: u64, mitigation: ErrorMitigationConfig) -> Result<Self> {
        Ok(Self {
            default_proxy,
    ...
    pub async fn handle_connection(
        ...
        match proxy_url {
            Some(proxy) => {
                self.handle_proxy_connection(intercepted_conn, client_stream, proxy).await
            }
            None => {
                self.handle_direct_connection(intercepted_conn, client_stream).await
            }
        }
    }
    ...
    async fn handle_proxy_connection(
        ...
        loop {
            match timeout(self.connect_timeout, TcpStream::connect(&proxy_addr)).await {
                Ok(Ok(stream)) => { upstream_stream_opt = Some(stream); break; }
                Ok(Err(e)) => {
                    warn!("Failed to connect to upstream proxy {}: {} (attempt {}/{})", proxy_addr, e, attempt + 1, max_attempts);
                }
                Err(_) => {
                    warn!("Timeout connecting to upstream proxy {} after {:?} (attempt {}/{})", proxy_addr, self.connect_timeout, attempt + 1, max_attempts);
                }
            }
            attempt += 1;
    ...
    let upstream_stream = if let Some(s) = upstream_stream_opt { s } else {
            ...
            if self.mitigation.try_direct_on_proxy_failure {
                ...
                return self.handle_direct_connection(intercepted_conn, client_stream).await;
            }
            if self.mitigation.rst_on_failure { self.send_rst_and_close(&mut client_stream).await; }
            return Err(TotanError::UpstreamProxy(format!("Connection to {} failed after {} attempts", proxy_addr, max_attempts)).into());
        };
    ...
    match url.scheme() {
            "http" => {
                self.handle_http_proxy(intercepted_conn, client_stream, upstream_stream).await
            }
            "socks5" => {
                self.handle_socks5_proxy(intercepted_conn, client_stream, upstream_stream).await
            }
            scheme => {
    ...
    async fn handle_direct_connection(
        ...
        let mut upstream_stream_opt: Option<TcpStream> = None;
        loop {
            match timeout(self.connect_timeout, TcpStream::connect(intercepted_conn.original_dest)).await {
                Ok(Ok(stream)) => { upstream_stream_opt = Some(stream); break; }
                Ok(Err(e)) => {
                    warn!("Failed to connect directly to {}: {} (attempt {}/{})", intercepted_conn.original_dest, e, attempt + 1, max_attempts);
                }
                Err(_) => {
                    warn!("Timeout connecting directly to {} after {:?} (attempt {}/{})", intercepted_conn.original_dest, self.connect_timeout, attempt + 1, max_attempts);
                }
            }
            attempt += 1;
            if attempt >= max_attempts { break; }
    ...
    let mut upstream_stream = if let Some(s) = upstream_stream_opt { s } else {
            if self.mitigation.rst_on_failure { self.send_rst_and_close(&mut client_stream).await; }
            return Err(TotanError::Network(format!("Direct connection to {} failed after {} attempts", intercepted_conn.original_dest, max_attempts)).into());
        };
        // Reduce latency
        let _ = client_stream.set_nodelay(true);
    ...
    async fn handle_http_proxy(
        ...
        if is_tls {
            ...
            let authority = format!("{}:{}", authority_host, intercepted_conn.original_dest.port());
            ...
            if response_str.starts_with("HTTP/1.1 200") || response_str.starts_with("HTTP/1.0 200") {
                ...
                return Err(TotanError::UpstreamProxy(
                    "HTTP proxy CONNECT request failed".to_string(),
                )
                .into());
            }
        } else {
            ...
            if let Err(e) = self
                .rewrite_and_forward_http_request(
                    &mut client_stream,
                    &mut upstream_stream,
                    &intercepted_conn,
                )
    ...
    async fn handle_socks5_proxy(
        ...
        if resp[0] != 0x05 || resp[1] != 0x00 {
            warn!("SOCKS5 server requires unsupported auth method: {:02x}", resp[1]);
            self.send_rst_and_close(&mut client_stream).await;
            return Err(TotanError::UpstreamProxy("SOCKS5 auth method unsupported".to_string()).into());
        }
        ...
        debug!("SOCKS5 CONNECT to {}:{}", host, intercepted_conn.original_dest.port());
            ...
            debug!("SOCKS5 CONNECT to {}:{}", v4, intercepted_conn.original_dest.port());
                    req.push(0x01); // ATYP=IPv4
                    req.extend_from_slice(&v4.octets());
                }
                std::net::IpAddr::V6(v6) => {
            debug!("SOCKS5 CONNECT to {}:{}", v6, intercepted_conn.original_dest.port());
        ...
        if head[0] != 0x05 {
            self.send_rst_and_close(&mut client_stream).await;
            return Err(TotanError::UpstreamProxy("Invalid SOCKS5 reply version".to_string()).into());
        }
        if head[1] != 0x00 {
            self.send_rst_and_close(&mut client_stream).await;
            return Err(TotanError::UpstreamProxy(format!("SOCKS5 CONNECT failed, code {:02x}", head[1])).into());
        }
    ...
    async fn rewrite_and_forward_http_request(
        &self,
        ...
        while header_end.is_none() {
            ...
            if n == 0 {
                return Err(TotanError::Network("Client closed before sending HTTP headers".to_string()).into());
            }
    ...
    let mut other_headers: Vec<String> = Vec::new();
        ...
        let host_for_abs = if let Some(h) = host_header.clone() { h } else {
            ...
            if port == 80 { host } else { format!("{}:{}", host, port) }
        };
        ...
        let (new_request_line, debug_url): (String, String) = if absolute_target_needed {
            let url = format!("http://{}{}", host_for_abs, if target.starts_with('/') { target } else { "/" });
            (format!("{} {} {}\r\n", method, url, version), url)
        } else {
            // already absolute or authority-form; keep as-is
            (format!("{} {} {}\r\n", method, target, version), target.to_string())
        };
